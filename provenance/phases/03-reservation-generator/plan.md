...
todos:
  - id: implement
    content: "Implement r9_pod_a_pipeline/make_reservation.py: argparse with start/end/selected-csv/name/user/output-script flags, load + dedup hosts from selected_nodes.csv, build scontrol command, print to stdout, optionally write a shell script"
    status: completed
  - id: doc
    content: Append a short 'Slurm reservation' subsection to r9_pod_a_pipeline/README.md showing the quickstart command
    status: completed
  - id: verify
    content: Run python make_reservation.py with defaults; verify stdout includes Flags=MAINT,IGNORE_JOBS, the auto-generated name contains temp_power_reduce, and Nodes= count matches unique hosts in selected_nodes.csv
    status: completed
isProject: false
---
